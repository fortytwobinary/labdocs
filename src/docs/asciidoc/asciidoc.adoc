...
http://control-1.fortytwobinary.lab/[home]
...
- The optional `scaledwidth` attribute is only used in DocBook block
  ...
  image::logo.png[scaledwidth="75%",alt="Company Logo"]
...
- The optional `align` attribute aligns block macro images
  ...
  image::tiger.png["Tiger image",align="left"]
...
This example:
  ...
  .Main circuit board
  image::layout.png[J14P main circuit board]

is equivalent to:

  image::layout.png["J14P main circuit board", title="Main circuit board"]
...
defaults to `./icons`.  If admonition icons are embedded using
